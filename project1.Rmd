...
# Executive summary
# Analysis
A machine learning algrithm based on the random forest approach including cross-validation (10x10) has been performed leading to an out-of-sample error of `1-0.9878` or `1.2%`.
## Loading data
...
dim(mytrain_red4)
```
reduced dimensions to 17. Now using the Random forest of the 'caret package' because it tries multiple values for several of the key model parameters. The choice is to do 10 repeats of 10-fold cross-validation. Furthermore computing an out of sample error:
```{r rfr4,dependson="red4"}
tc <- trainControl("repeatedcv", number=10, repeats=10, classProbs=TRUE, savePred=T) 
modelrf4 <- train(classe ~ ., method='rf',data=mytrain_red4,trControl = tc)
